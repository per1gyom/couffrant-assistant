...
# ID du dossier Archive de Guillaume (récupéré via /list-mail-folders)
...
def perform_outlook_action(action: str, params: dict, token: str) -> dict:
    ...
    if action == "get_message_body":
        # Lire le corps COMPLET d'un mail
    ...
    if action == "create_reply_draft":
        ...
        return {
            ...
            "action": action,
            "draft_id": draft_id,
            "message": "Brouillon créé et prêt à envoyer.",
        }
    ...
    if action == "delete_message":
        ...
        return {
            ...
            "action": action,
            "message": "Mail supprimé définitivement." if success else "Échec de la suppression.",
        }
...
<div style="font-family:Arial, sans-serif; font-size:14px; color:#222;">
    ...
    if action == "create_calendar_event":
        try:
            ...
            return {
                ...
                "message": f"RDV '{params.get('subject')}' créé avec succès.",
            }
        except Exception as e:
    ...
    if action == "update_calendar_event":
        ...
        if "subject" in params:
            update_body["subject"] = params["subject"]
        if "start" in params:
            update_body["start"] = {"dateTime": params["start"], "timeZone": "Europe/Paris"}
        if "end" in params:
            update_body["end"] = {"dateTime": params["end"], "timeZone": "Europe/Paris"}
        if "body" in params:
            update_body["body"] = {"contentType": "Text", "content": params["body"]}
        try:
            _graph_patch(token, f"/me/events/{event_id}", update_body)
            return {"status": "ok", "action": action, "message": "RDV mis à jour."}
        except Exception as e:
            return {"status": "error", "action": action, "message": f"Échec mise à jour : {str(e)}"}
    ...
    if action == "delete_calendar_event":
        ...
        return {
            "status": "ok" if success else "error",
            "message": "RDV supprimé." if success else "Échec suppression RDV."
        }
    ...
    if action == "send_teams_message":
        ...
        try:
            ...
            return {"status": "error", "action": action, "message": f"Échec envoi Teams : {str(e)}"}

    if action == "get_teams_chat_messages":
        chat_id = params["chat_id"]
        top = params.get("top", 20)
        data = _graph_get(token, f"/me/chats/{chat_id}/messages", params={"$top": top})
        return {
            "status": "ok",
            "action": action,
            "items": data.get("value", []),
    ...
    if action == "find_teams_user":
        # Trouver un utilisateur Teams par nom pour récupérer son chat
        name = params["name"]
        data = _graph_get(
            token,
            "/users",
            params={"$filter": f"startswith(displayName,'{name}')", "$select": "id,displayName,mail"}
        )
        return {
            "status": "ok",
            "action": action,
    ...
    if action == "move_onedrive_file":
        ...
        try:
            ...
            return {"status": "error", "action": action, "message": f"Échec déplacement : {str(e)}"}
    ...
    if action == "create_onedrive_folder":
        ...
        try:
            ...
            return {"status": "error", "action": action, "message": f"Échec création dossier : {str(e)}"}
    ...
    if action == "delete_onedrive_file":
        ...
        return {
            "status": "ok" if success else "error",
            "message": "Fichier supprimé." if success else "Échec suppression fichier."
        }
    ...
    if action == "list_todo_tasks":
        ...
        all_tasks = []
        for lst in lists_data.get("value", [])[:3]:  # 3 premières listes
    ...
    if action == "create_todo_task":
        ...
        if not lists_data.get("value"):
            return {"status": "error", "message": "Aucune liste de tâches trouvée."}
        ...
        try:
            ...
            return {"status": "error", "action": action, "message": f"Échec création tâche : {str(e)}"}
