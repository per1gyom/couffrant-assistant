...
def _static_signature(username: str) -> str:
    """Signature statique de fallback — si aucune signature configuree en DB.

    Avant : signature Guillaume hardcodee (Couffrant Solar, banner, telephone,
    site web). Charlotte ou tout autre tenant aurait herite de cette signature
    par erreur si rien n etait en DB pour son user.

    Apres : signature minimaliste neutre. L utilisateur configure sa vraie
    signature dans Mes signatures (onglet user_settings). Tant qu il n a rien
    fait, on n insere RIEN (chaine vide) — le mail part sans signature
    plutot qu avec un nom qui n est pas le sien."""
    return ""
