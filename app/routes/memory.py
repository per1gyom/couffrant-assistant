...
def build_memory(request: Request, user: dict = Depends(require_user)):
...
def memory_status(request: Request, user: dict = Depends(require_user)):
    username = user["username"]
    tenant_id = user["tenant_id"]
    conn = None
    try:
    ...
    return {
        ...
        "tenant_id": tenant_id,
        "niveau_1": {
            "resume_chaud": {"exists": bool(row and row[0])},
...
@router.get("/contacts")
def list_contacts_endpoint(request: Request, user: dict = Depends(require_user)):
    return get_all_contact_cards(tenant_id=user["tenant_id"])
...
def learn_style(request: Request, payload: dict = Body(...), user: dict = Depends(require_user)):
    text = payload.get("text", "")
    if not text:
        return {"error": "Texte manquant"}
    save_style_example(
        situation=payload.get("situation", "mail"),
        example_text=text,
