...
La logique est déléguée à aria_context.py (construction du contexte)
et aria_actions.py (exécution des actions).
"""
import os
import re
import io
...
def _build_user_content(payload: RayaQuery):
    ...
    if payload.file_type.startswith("image/"):
        parts.append({"type": "image", "source": {"type": "base64", "media_type": payload.file_type, "data": payload.file_data}})
        parts.append({"type": "text", "text": f"[Image jointe{file_name_info}]\n{payload.query}" if payload.query else f"[Image jointe{file_name_info}] Analyse ce document."})
    elif payload.file_type == "application/pdf":
        parts.append({"type": "document", "source": {"type": "base64", "media_type": "application/pdf", "data": payload.file_data}})
        parts.append({"type": "text", "text": f"[PDF joint{file_name_info}]\n{payload.query}" if payload.query else f"[PDF joint{file_name_info}] Analyse ce document."})
    else:
        parts.append({"type": "text", "text": f"[Fichier joint{file_name_info}]\n{payload.query}"})
    return parts
