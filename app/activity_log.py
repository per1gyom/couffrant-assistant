...
def log_activity(username: str, action_type: str, action_target: str = None,
                 action_detail: str = None, tenant_id: str = None) -> None:
    """Log une action utilisateur. Non bloquant (silencieux si erreur)."""
    try:
        conn = get_pg_conn()
        c = conn.cursor()
        c.execute("""
            INSERT INTO activity_log (username, tenant_id, action_type, action_target, action_detail)
            VALUES (%s, %s, %s, %s, %s)
