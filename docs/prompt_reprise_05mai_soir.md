...
**Date de generation :** Mardi 5 mai 2026 (fin de matinee 12h, post-marathon 4h)
...
### Mail (6 boites - **contact@ supprimee le 05/05 matin, a recreer**)

| ID | Boite | Type | Permission |
|---|---|---|---|
...
**Note** : conn 12 (Contact Couffrant Solar) a ete supprimee le 05/05 matin suite a un probleme Microsoft Authenticator empechant la reconnexion. Sera recreee plus tard avec authentification directe sur le compte contact@ (compte autonome, pas alias).
...
## Session matin 05/05/2026 (8h-10h) — Audit identite tokens OAuth + nettoyage

**Theme** : suite au bootstrap contact@ qui n avait remonte que 111/1418 mails, audit complet revelant un probleme architectural sur les tokens Outlook.

### Cause racine identifiee

Le bootstrap utilisait `get_valid_microsoft_token(username)` (table legacy `oauth_tokens` : 1 seul token par user/provider). Avec 2 boites Outlook (guillaume@ et contact@), elles utilisaient le MEME token (le dernier OAuth = guillaume@). Donc `/me/mailFolders/Inbox` retournait l inbox de guillaume@ alors qu on croyait bootstraper contact@.

Le systeme V2 (token par-connexion via `tenant_connections.credentials`) existait deja dans `connection_token_manager.py` mais les jobs Outlook ne l utilisaient pas. Gmail l utilisait deja correctement -> les 5 boites Gmail etaient OK.

### Commits pousses ce matin
...
| `ed71b50` | feat(diag-ui): page HTML lisible `/admin/mail/diag/identities-page` |

### Diagnostic confirme via page HTML

7 connexions auditees, 0 mismatch cote OAuth pur. Mais conn 12 (contact@) avait son token V2 expire depuis le 28/04 (jamais utilise par le legacy donc jamais refresh). Le pattern bizarre des donnees (87% des mails contact@ avaient from=guillaume@) venait bien du contournement par le legacy.

### Nettoyage donnees execute en base (mode dry-run puis COMMIT validation Guillaume)
...
- 0 doublon exact (les `id` Microsoft Graph differaient entre les 2 boites)
- guillaume@ : passe de 1027 -> 1138 mails actifs, ratio noeuds graphe 1138/1138 ✓

### Connexion 12 supprimee

Apres mon fix V2, conn 12 est passee `down` (refresh KO sur token expire). Guillaume a essaye de reconnecter mais Microsoft Authenticator bloquait. Il a supprime la connexion 12 depuis son panel super admin pour la recreer plus tard. Cascade DB confirmee : `connection_health` et `connection_assignments` aussi videes, 0 mail orphelin.

### A faire au moment de la reconnexion contact@

1. Authentification Microsoft AVEC le compte contact@ directement (compte autonome confirme par Guillaume, pas alias) -> ouvrir Safari en navigation privee pour eviter la reconnexion auto sur guillaume@
2. Verification via `/admin/mail/diag/identities-page` (doit montrer mismatch=0)
3. Bootstrap historique declenche depuis le panel super admin (devrait remonter ~1031 mails Inbox+SentItems)
...
## Chantiers suivants (prioritises au 05/05 12h)

### Termines ce matin (05/05 8h-12h) - 7 commits

- ✅ Diagnostic OAuth tokens + page UI (`ed68f77`, `ed71b50`)
- ✅ Fix V2 Outlook par-connexion (`8eb31ab`) - bootstrap et delta-sync
- ✅ Filtre bulk reduit (`0ff1b97`) - factures/commandes/livraisons gardees
- ✅ Alertes 3 niveaux (`94d3ab7`) - rouge/orange = vrais problemes uniquement
- ✅ Recon Gmail pommes vs pommes (`53272ad`) - tient compte du filtrage Haiku
- ✅ `mail_id` expose a Raya dans search_mail (`3056fe7`)
- ✅ UX 1.2 backend - aria_memory pre-cree (`a60d31b`) - pending_actions ont conversation_id

### En cours ou prochaine etape

**EN COURS (05/05 apres-midi)** :
- **UI dropdown permissions dans le panel admin** (`admin_connexions.html`) : remplacer l affichage lecture seule par un menu deroulant 3 niveaux avec bouton Sauver, reserve super_admin. L endpoint `PUT /tenant/connections/{id}` existe deja en backend (depuis 18/04), il manque juste le frontend.

**A FAIRE plus tard** :

1. **Reconnexion contact@couffrant-solar.fr** (Guillaume, quand probleme Microsoft Authenticator regle) : recreer la connexion en s authentifiant avec le compte contact@ directement. Verifier via `/admin/mail/diag/identities-page`. Lancer bootstrap historique. Le code V2 (`8eb31ab`) garantit qu il utilisera le bon token cette fois.

2. **Permissions par USER** (au-dela du tenant_admin) — Charlotte arrive le 7 mai, on a besoin de differencier "Guillaume = tout", "Charlotte = lire+ecrire", "Stagiaire = lire seul" sur les memes connexions tenant.

3. **Relancer les bootstraps Gmail** (optionnel) : recuperer les mails business filtres a tort par l ancien filtre bulk avant le commit `0ff1b97`. Estimation : ~941 mails legitimes loupes sur per1.guillaume@gmail.com (factures Stripe, etc).

4. **Audit Opus des regles** : ameliorer `_job_opus_audit` pour detecter doublons semantiques + contradictions (les 3 regles 71, 76, 187 etaient toutes contradictoires entre elles et avec la 188 -- l audit n a rien detecte).

5. **`activity_log` casse depuis 21/04** : MAX(created_at) = `2026-04-21T07:58:57`, 294 entrees au total. Plus aucune action tracee depuis. Chantier independant.

6. **Suppression du legacy `oauth_tokens`** : maintenant que Outlook utilise V2, la table legacy n est plus necessaire. La supprimer evitera qu un futur job se branche dessus par erreur.
...
- **Tool `query_database`** : lecture seule sur Postgres avec perimetre filtre (pas de credentials, pas de tokens chiffres).
- **Tool `read_code`** : lecture du code source du repo.
- **Tool `web_search` etendu** : pour chercher des infos externes sur internet.
- **Encourager le doute legitime** dans le prompt systeme.
- **Ameliorer les retours d erreur des tools**.

**Securite** : **JAMAIS d ecriture pour Raya via ces nouveaux tools**. Si elle suggere une amelioration intelligente, Guillaume me la transmet, on la met en place ensemble.

### Plus loin

7. **Proactivite** (phase finale, apres tout le reste).
