...
### Phase 7 ✅ | Phase 8 ✅ | Refactorings 5 ✅ | Fixes 10+ ✅

### Session 13/04 — tâches terminées
- PWA-FIX ✅ — Icônes PNG Pillow + manifest + apple-touch-icon (3 commits)
- DIAG-ENDPOINTS ✅ — GET /admin/diag (MS, Gmail, Odoo, Twilio, ElevenLabs) + card panel admin
- GMAIL-FIX ✅ — OAuth2 complet (get_gmail_auth_url, exchange_code_for_tokens, callback redirige /chat, bouton "Connecter Gmail" dans tiroir admin)
- FORGOT-PASSWORD ✅ — Self-service email reset, rate limit 3/h, timing constant, anti-énumération

## 3. PROMPTS EN ATTENTE

Aucun. Zéro dette.

## 4. PROCHAINES ÉTAPES (suite immédiate)

### ⭐ En cours — Compte Guillaume opérationnel + mobile

**À FAIRE MAINTENANT :**
- [ ] Tester le diagnostic /admin/diag en live (Guillaume)
- [ ] Connecter Gmail : visiter /login/gmail → autoriser → vérifier tokens
- [ ] Diagnostiquer Odoo : vérifier variables Railway, tester via /admin/diag
- [ ] Tester PWA sur iPhone : "Ajouter à l'écran d'accueil" dans Safari
- [ ] Tester Twilio : configurer webhook URL dans console Twilio, envoyer test WhatsApp
- [ ] Variable Railway à ajouter : GMAIL_REDIRECT_URI, SMTP_HOST/USER/PASS

**Outils de création (à coder) :**
- DALL-E + Pillow (images)
- Excel (openpyxl), PDF (reportlab)
- Posts LinkedIn

### Priorité 2 — Beta Charlotte (tenant "juillet")
### Priorité 3 — Tenant DEMO (prospection) avec bouton purge
### Priorité 4 — UI ergonomie + audit performance

## 5. PRINCIPES ARCHITECTURAUX

- Intelligence collective, collaboration inter-Rayas, imports lazy
- Fichiers < 15k, prompts architecte (Opus = QUOI, Sonnet = CODE)

## 6. HISTORIQUE DES SESSIONS

### Session 13/04/2026 (continuation)
PWA-FIX (3 commits), DIAG-ENDPOINTS (2 commits), GMAIL-FIX (3 commits), FORGOT-PASSWORD.
Audit complet des fichiers Sonnet (admin_panel.html 59k vérifié OK).
