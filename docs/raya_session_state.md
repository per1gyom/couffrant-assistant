...
- Cache-bust JS/CSS : **v=38** (admin-panel.js) / **v=79** (chat)
...
- Chantier mémoire 4 couches (18/04/2026) : Bloc 1 (migrations + semantic_graph) **FAIT** (`2dd6402`), Bloc 2 (vectorisation Odoo) **FAIT** (`3b69833`), Bloc 2.5 (surveillance limites + alertes) **FAIT** (`7e29d4c`), Bloc 3 (webhooks Odoo) **FAIT code** — endpoint `/webhooks/odoo/record-changed` actif, doc config `docs/odoo_webhook_setup.md`. Bloc 4 (hybrid search + rerank + tag ODOO_SEMANTIC) **FAIT** (`ac2dab4`), Fixes D+A+B2+B3+C **FAITS** (`340ada8`, `4808fe8`, `78d9367`).
- **SCANNER UNIVERSEL ODOO — EN COURS** (plan finalisé 18/04 après-midi, implémentation 18/04 soirée) : voir `docs/raya_scanner_universel_plan.md` (1142 lignes). Plan complet validé en 10 questions tranchées avec Guillaume. Phases 1+2+3+6 codées (commits fbab358, 1e31a4c, 6f72c7f, b8378aa). Phase 3 (scan P1 avec vectorisation) **en attente de relance finale** après incident Railway (volume saturé → resize 1→5 Go + bug Live Resize beta nécessitant intervention manuelle du support Railway, résolu par employé brody). Garde-fous actifs (limits 5k records sur product.template, 10k sur mail.message/tracking, skip des référentiels géo). Scan ralenti à 500ms/batch + 50 records/batch pour éviter que Railway tue l'app. 32 manifests P1+P2 générés automatiquement. À lancer une dernière fois avant dodo ce soir pour validation finale.
